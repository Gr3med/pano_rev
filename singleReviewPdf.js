...
export async function createSingleReviewPdf(data, __dirname) {
    const logoBase64 = await fs.readFile(path.join(__dirname, 'logo.jpg'), 'base64');
    ...
    <head>
        ...
        <style>
            ...
            .comments-section { margin-top: 20px; padding: 15px; background-color: #f9f9f9; border-radius: 8px; }
    ...
    <body>
        <div class="card">
            <div class="header">
                <img src="${logoUri}" alt="Hotel Logo" width="120">
            ...
            ${data.comments ? `<div class="section-title">مقترحات</div><div class="comments-section"><p>${data.comments}</p></div>` : ''}
            <div class="footer">تم الإنشاء بتاريخ ${today}</div>
        </div>
    </body></html>`;
